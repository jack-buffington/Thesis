function testTGM_FMAandPSRworkflow()
   ...
   % To finish up, it draws a comparison of the original and estimated source maps.
   close all
   clc
   ...
   % Use the triangulated gradient method to estimate the source location based on
   % the measurements.
   estimates = TGM(measurements, triangles);
   
   
   % Place some emitters
   emitters = getEmitters(mapRegion, emitterSpacing, estimates, exclusionRadius);
   ...
   s = subplot(2,3,3);
   plot(estimates(:,1),estimates(:,2),'.k');
   hold on
   plot(actualEmitters(:,1),actualEmitters(:,2),'.r');
   s.XLim = [0 10];
   s.YLim = [0 10];
   set(gca,'Ydir','reverse')
   title('Estimated source locations');
   plot(measurements(:,1),measurements(:,2),'b.');
   axis equal
   
   descr = {'For the plot above:';
       'red: actual sources';
       'blue: measurements';
       'black: estimated sources';};
